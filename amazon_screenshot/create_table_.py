...
def pending_links():
    cursor.execute(f"SELECT COUNT(*) FROM {db.pdp_link_table} WHERE status = 'Done' or status = 'Finally_Done'")
    done_count = cursor.fetchone()[0]
    print(f"Done count: {done_count}")

    # if done_count >= 800:
    if done_count >= 640:
        # update_query = f"UPDATE {db.pdp_link_table} SET status = 'Pending' WHERE status = 'Done' or status = 'Finally_Done'"
        update_query = f"UPDATE {db.pdp_link_table} SET status = 'Done' WHERE status = 'Finally_Done'"
