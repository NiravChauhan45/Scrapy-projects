    ...
    if response:
        ...
        if variation_list:
            ...
            for index, variant in enumerate(variation_list):
                ...
                item['hash_id'] = fuc.get_HashId(product_url, item['SKU'])
                # fuc.make_pagesave(response, item['hash_id'])
                # fuc.insertItemToSql(item, product_url, connection, cursor)

            # Todo: Done-Pending
            try:
                sql = f"UPDATE {db.pdp_links} SET status = %s WHERE product_url = %s"
                values = ('Pagesave_Done', product_url)
...
if __name__ == "__main__":
    # Todo: Using for loop
    sql_query = f"SELECT * FROM {db.pdp_links} WHERE status='Pending';"
    ...
    # with ThreadPoolExecutor(max_workers=30) as executor:  # adjust worker count as needed
