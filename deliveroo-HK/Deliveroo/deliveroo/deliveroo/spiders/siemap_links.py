...
class SiemapLinksSpider(scrapy.Spider):
    name = "siemap_links"
    ...
    def parse(self, response):
        ...
        url = "http://api.scraperapi.com?api_key=de51e4aafe704395654a32ba0a14494d&url=https://deliveroo.hk/en/sitemap_restaurant/1.xml"
        response=requests.request("GET",url=url)
        data=re.findall("<loc>(.*?)</loc>",response.text)
        for d in data:
            print(d)
            url2=d+"?collection=all-restaurants"
            url3 = f"http://api.scraperapi.com?api_key=de51e4aafe704395654a32ba0a14494d&url={url2}"
            payload = {}
            headers = {
            ...
            print("URRLLL",url)
            response2 = HtmlResponse(body=response2.text,url="", encoding='utf-8')
            data_json = response2.xpath('//script[@type="application/json"]//text()').get()
            try:
                    ...
                    for items in item_loop_data:
                        ...
                        try:
                            for ii in items_more:
                                ...
                                url_rest = "https://deliveroo.hk/en" + ii['target']['restaurant']['links']['self']['href']
                                ...
                                try:
                                    ...
                                    insert_db = f"insert into sitemap_links ( " + fields + " ) values ( '" + values + "' )"
...
if __name__ == '__main__':
    execute("scrapy crawl siemap_links".split())