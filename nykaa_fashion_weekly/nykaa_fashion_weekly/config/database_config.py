...
class ConfigDatabase():
    ...
    def insertItemToSql(self, item):
        global insert_db, value_list
        try:
            # Ensure valid field names
            safe_fields = []
            for f in item.keys():
                safe_name = re.sub(r"[^\w]+", "_", f)  # Replace invalid chars
                safe_fields.append(f"`{safe_name}`")

            placeholders = ["%s"] * len(item)
            field_list = ",".join(safe_fields)
            placeholder_str = ",".join(placeholders)

            # Safely quote table name with backticks
            insert_db = f"INSERT IGNORE INTO `{self.table}` ({field_list}) VALUES ({placeholder_str})"
            ...
            print(f"❌ MySQL Error: {e.args}")
            print(f"Query: {insert_db}")
