...
query = f"SELECT * FROM {db.amazon_pdp_link} where status='Pending' and `Amazon Fresh`!='' limit 3"
    ...
    async with async_playwright() as p:
        for result in results:
            ...
            if product_url:
                ...
                service_availibility = response.xpath(
                ...
                if not service_availibility:
                    try:
                        if pid != 'N/A':
                            ...
                            filepath = fr"F:\Nirav\Project_page_save\Amazon_Fresh_Screenshot\{db.current_date}"
                    ...
                    try:
                        ...
                        if not count:
                            count = response.xpath("//ul[@class='a-nostyle a-list-link']/li[@class='a-dropdown-item'][last()]/a/text()")
                    except:
                        count = 'N/A'
                else:
                    stock_avaibility = 'Not listed'
                ...
                try:
                    on_site_sku_name = response.xpath('//h1[@id="title"]/span[@id="productTitle"]/text()').get(
                        '').strip()
                except:
                ...
                await page.click('//span[@id="GLUXZipUpdate-announce"]')
                await page.wait_for_timeout(1200)
                await page.fill('//input[@id="twotabsearchtextbox"]', name_of_the_product)
                ...
                try:
                    ...
                    pack_shot_name = str(
                        int(hashlib.md5(
                            bytes(str(name_of_the_product) + str(new_zipcode) + str(current_date), "utf8")).hexdigest(),
                            16) % (
                                10 ** 10))
