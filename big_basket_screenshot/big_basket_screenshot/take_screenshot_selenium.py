...
def fetch_page(driver, results, conn):
    ...
    for page_id, pdp_name, pdp_url, pincode in results:
        ...
        while pincode_set:
            try:
                ...
                try:
                    ...
                    if match_zipcode:
                        if sys.argv[5] in match_zipcode or sys.argv[5] in driver.page_source:
                ...
                try:
                    ...
                    if from_input:
                        from_input.click()
                        from_input.send_keys(sys.argv[5])
                        time.sleep(2)
        ...
        ss_path = db.screenshot_filepath + screeshot_path
        # Save the screenshot
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 5:
        print("Usage: python script.py <port> <start_id> <end_id> <num_tabs> <pincode>")
    ...
    cur = conn.cursor()
    # cur.execute(f'SELECT Id, `Name_of_the_Product`,`big_basket`,`zipcode` FROM {db.pdp_link_table} WHERE `status`="pending" and `zipcode`="110001" and `zipcode`="{pincode}";')
    cur.execute(
