...
# co.add_extension(r'C:\Users\Admin\AppData\Local\Google\Chrome\User Data\Default\Extensions\chphlpgkkbolifaimnlloiipkdnihall')
...
def fetch_page(tab, urls, conn):
    ...
    while pincode_set:
        try:
            ...
            if match_zipcode:
                if sys.argv[5] in match_zipcode or sys.argv[5] in res._text:
                    print('nirvo') #_______________________________stop here _____________________
                    # pincode_set = False
                    break
                else:
                    pass
    ...
    for pdp_name, page_id, pdp_url, pincode in urls:
        if pdp_url:    #_________________________END HERE_______________________________
            try:
                tab.get(pdp_url)
            except Exception as e:
...
if __name__ == '__main__':
    try:
        create_table_qury.pending_links()
    except Exception as e:
        print(e)

    try:
        create_table_qury.create_table()
    except Exception as e:
        print(e)
