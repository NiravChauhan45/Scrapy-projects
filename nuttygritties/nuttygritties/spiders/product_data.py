...
from nuttygritties.items import NuttygrittiesItem
import pandas as pd
...
class ProductDataSpider(scrapy.Spider):
        ...
        for result in results:
            ...
            hash_key = int(hashlib.md5(
                bytes(
                    ...
                    "utf8")).hexdigest(),
                           16) % (10 ** 18)
            page_save_products = "D:\\Nirav_page_save\\nuttygritties\\page_save_product_page\\"
            access_path = f'{page_save_products}{str(hash_key)}.html'
            if os.path.exists(access_path):
                yield scrapy.Request(url=f'file:///{access_path}',
                                     dont_filter=True,
                                     callback=self.parse,
                                     meta={'platform': platform, 'hash_key': hash_key, 'Brand': Brand,
                                           'category': category, 'SKU': SKU, 'weight': weight,
                                           'product_name': product_name, 'product_id': product_id, 'url': url})
            else:
                cookies = {
                    'secure_customer_sig': '',
                    'localization': 'IN',
    ...
    def parse(self, response, **kwargs):
        ...
        platform = response.meta.get('platform')  # j_data.get('vendor')
        ...
        try:
            ...
            unit_price = round(float(selling_price) / float(weight), 2)
