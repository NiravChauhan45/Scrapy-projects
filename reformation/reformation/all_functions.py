...
def getPid(product_url, response):
    try:
        product_id = product_url.split('/')[-1]
        product_id = product_id.split(".")[0]
        return product_id
    except:
        product_id = response.xpath('//div[@data-action="Product-Show"]/@data-querystring').get()
        if product_id:
...
def getShortDescription(json_data, response):
    short_description = json_data.get('description')
    if short_description:
        short_description = cleanText(short_description)
        return short_description
    else:
        short_description = response.xpath('//div[@data-product-component="short-description"]/text()').get()
        if short_description:
            short_description = cleanText(short_description)
            return short_description
        else:
...
def getSalePrice(response, variant):
    try:
    ...
    if sale_price:
        ...
        sale_price = response.xpath('//div[@class="pdp__header"]//div[@class="price__sales sales "]//@content').getall()
        if sale_price:
            if len(sale_price) == 2:
            ...
            if sale_price:
                return cleanText(sale_price).replace("$", "")
            else:
                return None
        else:
            sale_price = response.xpath(
                '//div[@class="pdp__header"]//div[@class="price__sales sales "]//span[@class="price--formated"]/text()').getall()
            if sale_price:
                sale_price = "".join(sale_price)
                return cleanText(sale_price).replace("$", "")
            else:
...
def getCurrency(response, variant):
    try:
        currency = variant.get('priceCurrency')
        if currency:
            return currency
    except:
        return "USD"
...
def getIsonSale(response):
    is_on_sale = response.xpath(
        '//div[@class="pdp__final-sale font-size--12 flex flex-nowrap margin-t--4 hidden"]').getall()
    if is_on_sale:
        return "No"
    else:
        return "Yes"
...
def getInStock(response, variant):
    try:
...
def getColour(response, variant):
    try:
        colour_name = variant.get('color')
    except:
        colour_name = ''
    if colour_name:
...
def getSize(response, variant, index):
    try:
...
def getHashID(pid, sku, colour, size):
    hash_id = int(hashlib.md5(bytes(str(pid) + str(sku) + str(colour) + str(size), "utf8")).hexdigest(), 16) % (
            10 ** 18)
    return str(hash_id)


def insertItemToSql(item, connection, cursor):
    try:
