...
def product_details(product_url):
    ...
    pagesave_path = fr"E:\Nirav\Project_page_save\bloomingdales\28_11_2025"
    ...
    if response:
        ...
        try:
            ...
            if json_data1:
                ...
                j_data = json_data1.get('pageData').get('product').get('product').get('messages').get(
                    'relationships').get('memberProductMap')
        except Exception as e:
            print(e)
        ...
        if json_data:
            item = dict()
            try:
                for index, variant in enumerate(j_data.values()):
                    ...
                    item['Price'] = fuc.getVPrice(selector, variant,variation_list[index])
                    item['Price Currency'] = fuc.getVPriceCurrency()
                    item['Sale Price'] = fuc.saleVPrice(selector, variant,variation_list[index])
                    item['Final Price'] = fuc.saleVPrice(selector, variant,variation_list[index])
                    if not item['Price']:
...
if __name__ == "__main__":
    # Todo: Using for loop
    # sql_query = f"SELECT * FROM {db.pdp_links} WHERE status='Pending' and product_url like '%1901065%';"
    sql_query = f"SELECT * FROM {db.pdp_links} WHERE status='Pending' and product_url like '%3802372%';"
    # sql_query = f"SELECT * FROM {db.pdp_links} WHERE status='Pending' and product_url like '%5237191%';"
