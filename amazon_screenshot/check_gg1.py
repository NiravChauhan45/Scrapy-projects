...
def page_save(conn, page_id,html):
    file_name = f'{save_path}\{page_id}.html'
    # Save the HTML content to a file
    with open(file_name, 'wb') as file:
        file.write(html.encode())
    print(f"Saved page to: {file_name}")
    # file_hash = hashlib.sha256(link.encode()).hexdigest()
    # zip_file_name = os.path.join(save_path, f'{file_hash}.zip')
    # # Save HTML content to a ZIP file
    # with zipfile.ZipFile(zip_file_name, 'w', zipfile.ZIP_DEFLATED) as zipf:
    #     zipf.writestr(f'{file_hash}.html', html)

    # print(f"Saved page to: {zip_file_name}")
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(f"UPDATE pdp_link_table SET status = 'done' WHERE id = %s", (page_id,))
        conn.commit()
        cur.close()
        conn.close()
...
def fetch_page(tab, urls, conn):
    ...
    for pdp_name, page_id, pdp_url, pincode in urls:
        if 'https://www.amazon.in' in pdp_url:
            tab.get(pdp_url)
            if pdp_url:
                try:
            ...
            time.sleep(2)
            tab.get_screenshot(path=current_date, name=f"{pid}_{pincode}_{current_date}.png", full_page=True)
            ...
            if sku_input:
                ...
                try:
                    time.sleep(2)
                    tab.get_screenshot(path=current_date, name=f"{page_id}_{pincode}_{current_date}.png", full_page=True)
                except Exception as e:
                    print(e)
                html=tab.html
                page_save(conn, page_id, html)
...
if __name__ == '__main__':
    ...
    cur.execute(
        f'SELECT Id, `Name of the Product`,`Amazon Fresh`,`zipcode`  FROM pdp_link_table WHERE id BETWEEN {start_id} AND {end_id} and zipcode="{pincode}" and status="pending"')
