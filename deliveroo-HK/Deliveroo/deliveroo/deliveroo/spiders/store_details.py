...
class DataSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # formatted_date="2024_10_07"
        mycursor = mydb.cursor()
        mycursor.execute(f"select * from {db.restaurant_links} where status = 'store'  limit {self.a},{self.b};")#restaurant_links_2025_01_21
        # mycursor.execute(f"SELECT * FROM `store` WHERE vendor_id='649683'")
        ...
        for d in myresult:
            ...
            if response.status_code ==200:
                ...
                if rest_name:
                        ...
                        try:
                            ...
                            for pd in p_data:
                                ...
                                if mm_data == "Popular with other people":
                                    other_cats.append(mm_data)
                        except Exception as e:
                            print(e)
                    category = " , ".join(other_cats)
                    cat_id=",".join(cat_id_list)
                    ...
                    basket_url = "https://api.hk.deliveroo.com/consumer/ugc-service/graphql/"
                    ...
                    #Todo: using scrape.do proxy
                    basket_url = f"http://api.scrape.do?token=aa48e53ef4934d95a56acecacaec8fe454ebf634a98&url={basket_url}&device=desktop"
                    ...
                    try:
                        rating_score=reviews_res['data']['reviewData']['layouts'][0]['ratingBreakdown']['averageRating']
                    ...
                    address=json_data['props']['initialState']['menuPage']['menu']['meta']['restaurant']['location']['address']['address1']
                    address_local=address
                    postal_code=json_data['props']['initialState']['menuPage']['menu']['meta']['restaurant']['location']['address']['postCode']
                    if postal_code == "None" or postal_code==None:
                        postal_code=""
                    country_code=json_data['props']['initialState']['menuPage']['menu']['meta']['restaurant']['location']['address']['country']
                    city=json_data['props']['initialState']['menuPage']['menu']['meta']['restaurant']['location']['address']['city']
                    request_location_Latitude=json_data['props']['initialState']['menuPage']['menu']['meta']['customerLocation']['lat']
                    request_location_Longitude=json_data['props']['initialState']['menuPage']['menu']['meta']['customerLocation']['lon']
                    delivery_time_unit = ""
                    ...
                    if category !="":
                        ...
                        item['status']="store"
                        yield item
                        sql = f"update {db.restaurant_links} set `status`='store_Done' where vendor_id={vendor_id};"#restaurant_links_2025_01_21
                        mycursor.execute(sql)
                        mydb.commit()
                else:
                    update_d = current_date.strftime("%Y_%m_%d")
                    sql = f"update {db.restaurant_links} set `status`='store_NF' where vendor_id={vendor_id};"#restaurant_links_2025_01_21
                    mycursor.execute(sql)
                    mydb.commit()

if __name__ == '__main__':
    execute("scrapy crawl s_data -a a=0 -a b=15000".split())