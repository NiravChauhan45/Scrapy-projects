...
from nykaa.items import Nykaa_N_PdpLinksItem


class ExtractPdpLinksSpider(scrapy.Spider):
    name = "new_pdp_links"
        ...
        for url in category_url_lst:
            if "https://" not in url:
                ...
                for brand_key, brand_value in brands_dict.items():
                    ...
                    for gender_key, gender_value in gender_filter.items():
                        url = (f"https://www.nykaafashion.com/rest/appapi/V2/categories/products?PageSize=50"
                               "&filter_format=v2&apiVersion=5&currency=INR&country_code=IN&deviceType=WEBSITE&sort"
                               f"=popularity&device_os=desktop&searchTerm={slug}&currentPage={page_count}&brand_filter={brand_value}"
                               f"&gender_filter={gender_value}")
                        yield scrapy.Request(url=url, headers=self.headers, callback=self.parse_1, dont_filter=True,
                                             meta={"slug": slug, "brand_key": brand_key,
                ...
                for brand_key, brand_value in brands_dict.items():
                    url = (
                        f"https://www.nykaafashion.com/rest/appapi/V2/categories/products?PageSize=50&filter_format=v2"
    ...
    def parse_1(self, response):
        ...
        if products_lst:
            if category_id:
                ...
                page_count = response.meta.get('page_count') + 1
                gender_value = response.meta.get('gender_value')
                url = (f"https://www.nykaafashion.com/rest/appapi/V2/categories/products?PageSize=50"
                       "&filter_format=v2&apiVersion=5&currency=INR&country_code=IN&deviceType=WEBSITE&sort"
                       f"=popularity&device_os=desktop&searchTerm={slug}&currentPage={page_count}&brand_filter={brand_value}"
                       f"&gender_filter={gender_value}")
...
if __name__ == '__main__':
    cmdline.execute(f"scrapy crawl {ExtractPdpLinksSpider.name}".split())
