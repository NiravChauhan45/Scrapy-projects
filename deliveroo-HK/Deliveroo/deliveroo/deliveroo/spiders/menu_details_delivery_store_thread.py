...
def thread_delivery(a, v):
    ...
    mycursor.execute(f"select * from {db.deliveroo_restaurant} where status = 'store'  limit {a},{v}")
    ...
    for d in myresult:
        ...
        for c in cat_id_list:
            ...
            try:
                ...
                menu_category = ""
            if menu_category != "":
                # path = f"C:\\Actowiz\\pagesave\\Deliveroo\\{formatted_date_pagesave}\\Menu_Store_Delivery\\"
                path = db.menu_store_delivery_path
                ...
                for i in items:
                    ...
                    if menu_category != "":
                        ...
                        try:
                            ...
                            insert_db = f"insert into {db.data_menu_delivery} ( " + fields + " ) values ( '" + values + "' )" #data_menu_delivery_{formatted_date}
                            mycursor.execute(insert_db)
                            mydb.commit()
                            print(insert_db)
                        except Exception as e:
                            print(str(e))
        sql = f"update {db.deliveroo_restaurant} set `status`='store_delivery_Done'  where vendor_id={vendor_id};"#deliveroo_restaurant_{formatted_date}
